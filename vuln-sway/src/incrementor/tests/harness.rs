...
async fn deploy_and_use_incrementor() {
    ...
    // Now you can use `incrementor_instance` to call methods on the Incrementor contract
    ...
    println!("contract id {:?}", incrementor_counter_id);
    ...
    let result = incrementor_instance
        ...
        .with_contracts(&[&counter_instance])
        .call()
        .await
        .unwrap();
